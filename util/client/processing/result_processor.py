...
class ResultProcessor:
    ...
    async def _handle_message(self, message: str) -> None:
        ...
        # 显示热词匹配情况
        replaced_hotwords = correction_result.replaced_hotwords
        context_hotwords = correction_result.potential_hotwords

        # 1. 显示完全匹配/已替换的热词
        if replaced_hotwords:
            # 提取热词文本
            replaced_names = [hw for hw, score in replaced_hotwords]
            console.print(f'    完全匹配：[green4]{", ".join(replaced_names)}')

        # 2. 显示潜在热词（从上下文热词中排除已替换的）
        if context_hotwords:
            replaced_set = {hw for hw, score in replaced_hotwords}
            potential_matches = [(hw, score) for hw, score in context_hotwords if hw not in replaced_set]
        ...
        if Config.llm_enabled:
            ...
            llm_result = await llm_process_text(
                ...
                return_result=True,
                paste=paste,
                matched_hotwords=context_hotwords  # 传递上下文热词给 LLM
            )
        else:
