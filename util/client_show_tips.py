...
6. 编辑 `config.py` ，可以配置服务端地址、快捷键、录音开关……

注意事项：

1. 当用户安装了 `FFmpeg` 时，会以 `mp3` 格式保存录音；当用户没有装 `FFmpeg` 时，会以 `wav` 格式保存录音
2. 默认的快捷键是 {Config.shortcut}，你可以打开 `core_client.py` 进行修改
3. MacOS 无法监测到 `caps lock` 按键，可改为 `right shift` 按键
    ''')
    console.print(Markdown(markdown), highlight=1);
    console.rule()
