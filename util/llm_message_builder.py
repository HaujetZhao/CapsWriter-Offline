...
class MessageBuilder:
    ...
    def build_messages(
        ...
        # 剪贴板内容（在这里读取，而不是从外部传递）
        if role_config.enable_clipboard_read:
            from util.llm_clipboard import get_clipboard_text, record_clipboard_usage
            clipboard_text = get_clipboard_text(role_config, role_name)
            if clipboard_text:
                user_content_parts.append(f"剪贴板内容：{clipboard_text}")
                # 记录使用的剪贴板内容，避免下一轮重复加入
                record_clipboard_usage(role_name, clipboard_text)
