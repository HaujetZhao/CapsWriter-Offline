...
![image-20240108115946521](images/image-20240108115946521.png)  
...
![image-20240108114351535](images/image-20240108114351535.png) 
...
![image-20230531221314983](images/image-20230531221314983.png)
...
1. 语音输入任一句话后，录音就会被保存到 `records/2023/06/assets` 路径下，以时间和识别结果命名，并将识别结果保存到 `records/2023/06/05.md` 文件中，方便我日后查阅
2. 例如我在 `keywords.txt` 中定义了关键词「健康」，用于随时记录自己的身体状况，吃完饭后我可以按住 `CapsLock` 说「健康今天中午吃了大米炒饭」，由于识别结果以「健康」关键词开头，这条识别记录就会被保存到 `records/2023/06/05-健康.md` 中
3. 例如我在 `keywords.txt` 中定义了关键词「重要」，用于随时记录突然的灵感，有想法时我就可以按住 `CapsLock` 说「重要，xx问题可以用xxxx方法解决」，由于识别结果以「重要」关键词开头，这条识别记录就会被保存到 `records/2023/06/05-重要.md` 中

![image-20230604144824341](images/image-20230604144824341.png)  
...
![image-20240108114558762](images/image-20240108114558762.png)  
...
2. 运行 `core_client.py` 脚本，它会打开系统默认麦克风，开始监听按键（`MacOS` 和 `Linux` 端需要 `sudo`）
